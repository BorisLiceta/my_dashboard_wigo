...
tipo_busqueda = st.sidebar.selectbox("Seleccione tipo de búsqueda", ["Marca", "Sede", "Asesor comercial", 
                                                                     "Método de Pago","Rango de precios"])  
...
if tipo_busqueda == "Marca":
    ...
    df_filtrado = df[df["marca"] == valor]                                   # Filtrar búsqueda por marca                                   # Filtrar búsqueda por marca  
...
elif tipo_busqueda == "Método de Pago":
    valor = st.sidebar.selectbox("Seleccionar Método de Pago", df["Método de Pago"].unique()) # Mostrar las marcas disponibles y sin repetir
    df_filtrado = df[df["tienda"] == valor]      
    
elif tipo_busqueda == "Rango de precios":
    ...
    df_filtrado = df[
        (df["precio_venta"] >= rango[0]) &
        (df["precio_venta"] <= rango[1])
    ] 
